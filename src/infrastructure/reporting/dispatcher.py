...
class ReportDispatcher:
    ...
    async def dispatch(
        ...
        for fmt in output_formats:
            ...
            if handler:
                sent = await handler(group_id, analysis_result, platform_id) or sent

        if not sent and "text" not in output_formats:
            logger.warning(f"[{trace_id}] 配置的报告格式均发送失败，回退到文本报告。")
            sent = await self._dispatch_text(group_id, analysis_result, platform_id)
    ...
    async def _dispatch_image(
        ...
        if not self._html_render_func:
            logger.warning(f"[{trace_id}] 未设置 HTML 渲染函数，无法生成图片报告。")
            return False
        ...
        logger.warning(f"[{trace_id}] Image dispatch failed.")
        return False
    ...
    async def _dispatch_html(
        ...
        logger.warning(f"[{trace_id}] HTML dispatch failed.")
        return False
